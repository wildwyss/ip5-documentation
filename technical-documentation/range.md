...
Ranges can process sequences of almost any size and require hardly any memory or computing power. They are therefore very well suited as building blocks for larger constructs. Because ranges only process sequences of numbers they can be used very versatile. Possible use cases are for example the lazy generation of numbers or to execute a certain action `n` times.

The example above from Haskell shows very nicely how a range can be processed further. For example, elements can be skipped or parts can be cut off. With further such operations the use cases of ranges are hardly limited.
...
In a range, it's pretty easy to get from the current step to the next. The selected step size is added to the current state. &#x20;

### Stop detection

To find out if a range has reached the end of its values, the current value (current) is compared with the defined end (right).&#x20;
...
To see the range in action head directly to the [Examples](range.md#examples) section.

{% hint style="success" %}
Since the range returns a specialized [Kolibri Iterator](iterator.md), all the [Kolibri Iterator functions](iterator.md#iterator-functions) can be used with the range as well! So feel free, to `map` over a range and than `reduce` it to a single value. Or do any other stuff with it that comes to your mind :).
{% endhint %}
